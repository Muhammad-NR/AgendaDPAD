...
import { Plus, ChevronLeft, ChevronRight, MapPin, Clock, X, Loader2, Pencil, Trash2, AlertTriangle, Calendar as CalIcon, AlignLeft, MoreVertical, Building2 } from 'lucide-react';
import Cookies from 'js-cookie';

export default function AgendaKunjunganPage() {
  ...
  // State Form (Ada institution_name)
  const [formData, setFormData] = useState({
    title: '', description: '', time: '', end_time: '', location: '', institution_name: ''
  });
  ...
  const executeDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/kunjungan/${deleteId}`, { method: 'DELETE' });
      if (res.ok) {
        fetchAgendas();
  ...
  const handleEdit = (agenda: any) => {
    ...
    setFormData({
      ...
      institution_name: agenda.institution_name || ''
    });
  ...
  const closeModal = () => {
    ...
    setEditingId(null);
    setFormData({ title: '', description: '', time: '', end_time: '', location: '', institution_name: '' });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingId ? `/api/kunjungan/${editingId}` : '/api/kunjungan';
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-500 mt-1">Jadwal tamu dan instansi luar.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* KOLOM KIRI: Kalender (Warna Teal) */}
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl border shadow-sm">
            <div className="grid grid-cols-7 mb-4 text-center text-teal-600 font-bold">
              ...
              {days.map((day, idx) => {
                ...
                return (
                  <div key={idx} onClick={() => setSelectedDate(dateStr)}
                    className={`h-24 border rounded-xl p-2 cursor-pointer transition-all flex flex-col justify-between ${isSelected ? 'ring-2 ring-teal-500 bg-teal-50' : 'hover:border-teal-300'} ${hasEvent ? 'bg-teal-50/50' : 'bg-white'}`}>
                    <span className={`text-sm font-bold ${isSelected ? 'text-teal-600' : 'text-gray-700'}`}>{day}</span>
                    {hasEvent && <div className="w-2 h-2 bg-teal-500 rounded-full self-end mb-1"></div>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-white p-6 rounded-3xl border shadow-sm h-full flex flex-col">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="font-bold text-gray-800 text-xl">Tamu Hari Ini</h3>
              <p className="text-sm text-gray-500 mt-1">{displayDate}</p>
            </div>
            {isAdmin && (
              <button onClick={() => { closeModal(); setIsModalOpen(true); }} className="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 shadow-lg shadow-teal-200">
            ...
            {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-teal-500 w-8 h-8"/></div> : 
             filteredAgendas.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-48 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                  <p className="text-gray-400 font-medium">Tidak ada kunjungan.</p>
                </div>
             ) : 
             filteredAgendas.map((agenda) => (
                <div key={agenda.id} className="relative p-6 bg-white rounded-2xl border border-gray-200 hover:border-teal-300 shadow-sm hover:shadow-md transition-all group">
                  {/* Garis Aksen Kiri (TEAL) */}
                  <div className="absolute left-0 top-4 bottom-4 w-1.5 bg-teal-500 rounded-r-full"></div>
                  
                  <div className="pl-4 pr-8">
                    {/* JUDUL BESAR */}
                    <h4 className="font-bold text-gray-900 text-lg leading-tight mb-2">{agenda.title}</h4>
                    
                    {/* NAMA INSTANSI (KHUSUS KUNJUNGAN) */}
                    <div className="flex items-center gap-2 text-teal-700 font-bold text-sm mb-3">
                      <Building2 className="w-4 h-4"/> 
                      {agenda.institution_name}
                    </div>
                    ...
                    <div className="flex flex-col gap-2 text-sm">
                      <div className="flex items-center gap-3 text-gray-700 font-medium">
                        <div className="p-1.5 bg-teal-50 rounded-md text-teal-600"><Clock className="w-4 h-4"/></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/30 backdrop-blur-sm p-4 transition-all">
          <div className="bg-white rounded-2xl w-full max-w-lg p-8 shadow-2xl animate-in fade-in zoom-in duration-300 border border-white/50">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{editingId ? 'Edit Kunjungan' : 'Catat Kunjungan'}</h2>
                <p className="text-gray-400 text-sm mt-1">Lengkapi detail kunjungan di bawah ini.</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="bg-teal-50 p-4 rounded-xl border border-teal-100 flex items-center justify-between">
                <span className="text-xs font-bold text-teal-500 uppercase tracking-wider">Tanggal</span>
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1 mb-1 block">Agenda / Keperluan</label>
                <input required type="text" className="w-full bg-gray-50 border-gray-200 border rounded-xl p-3 focus:ring-2 focus:ring-teal-500 focus:bg-white outline-none transition-all font-medium text-gray-900 placeholder:text-gray-400"
                  value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
              </div>

              {/* INPUT NAMA INSTANSI (KHUSUS KUNJUNGAN) */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1 mb-1 block">Nama Instansi / Tamu</label>
                <input required type="text" placeholder="Contoh: Dinas Pendidikan Sleman" className="w-full bg-gray-50 border-gray-200 border rounded-xl p-3 focus:ring-2 focus:ring-teal-500 focus:bg-white outline-none transition-all font-medium text-gray-900 placeholder:text-gray-400"
                  value={formData.institution_name} onChange={e => setFormData({...formData, institution_name: e.target.value})} />
              </div>

              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1 mb-1 block">Mulai</label>
                  <input required type="time" className="w-full bg-gray-50 border-gray-200 border rounded-xl p-3 focus:ring-2 focus:ring-teal-500 focus:bg-white outline-none transition-all font-medium text-gray-900"
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1 mb-1 block">Lokasi / Ruangan</label>
                <input type="text" className="w-full bg-gray-50 border-gray-200 border rounded-xl p-3 focus:ring-2 focus:ring-teal-500 focus:bg-white outline-none transition-all font-medium text-gray-900 placeholder:text-gray-400"
                  value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1 mb-1 block">Catatan Tambahan</label>
                <textarea className="w-full bg-gray-50 border-gray-200 border rounded-xl p-3 focus:ring-2 focus:ring-teal-500 focus:bg-white outline-none transition-all font-medium text-gray-900 h-24 placeholder:text-gray-400 resize-none"
                  value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
              </div>
              
              <button disabled={isSubmitting} type="submit" className="w-full bg-teal-600 text-white py-3.5 rounded-xl font-bold hover:bg-teal-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-teal-200">
